...
def predict_multioutput_tabpfn(
...
def run_one(
    ...
    cfg = RunConfig(
        ...
        notes="Five-fold TabPFN classification ensemble for ordinal sensory targets.",
    )
    ...
    def predict(_Xtr: pd.DataFrame, _ytr: pd.DataFrame, _X: pd.DataFrame, apply_copy_through: bool, desc: str) -> np.ndarray:
        ...
        pred = predict_multioutput_tabpfn(
            _Xtr,
            _ytr[target_cols],
            _X,
    ...
    print(f"[cross_validated_tabpfn] oof filled rows: {int(oof_filled_rows.sum())}/{len(oof_filled_rows)}")
    print(f"[cross_validated_tabpfn] imputed cells (all targets): {imputed_all}")
    print(f"[cross_validated_tabpfn] imputed cells (sensory targets): {imputed_sensory}")
    ...
    test_avg = test_sum / float(CV_SPLITS)
...
def main() -> None:
    ...
    LOGGER = logging.getLogger("cross_validated_tabpfn")
    ...
    print(f"[cross_validated_tabpfn] done -> {out_dir}")
