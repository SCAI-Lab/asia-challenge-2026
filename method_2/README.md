# Method 2 — Five-seed TabPFN probability ensemble
...
Cross-validated ensembles and full-training seed ensembles are not the same thing.

Method 2 was kept because it provides a genuinely different modeling strategy:
...
## Inputs and included submission

Place the authorized Track 2 inputs in the repository-level `data/` directory:
...
The preserved Method 2 submission is
[`seed_ensemble_submission.csv`](data/submissions/seed_ensemble_submission.csv).
...
### Method 2
```bash
python method_2/scripts/train_seed_ensemble.py --do-cv 1 --n-splits 5
```
