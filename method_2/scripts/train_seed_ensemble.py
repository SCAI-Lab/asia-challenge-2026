...
from utils.evaluation import save_oof_npz
from utils.metrics import compute_metrics
from utils.weighted_metrics import compute_wrmse_imputed_only
from utils.discrete_tabpfn import build_preprocessor
from utils.runtime import RunConfig, ensure_dir, make_run_id, utc_now_iso, write_json
...
def _predict_seed_ensemble_probabilities(
    Xtr: pd.DataFrame,
    ytr: pd.DataFrame,
    Xte: pd.DataFrame,
    target_cols: List[str],
    ...
    seed_iter = tqdm(seeds, desc=f"{log_prefix.strip()}seeds".strip(), leave=False)
    for seed in seed_iter:
        ...
        target_iter = tqdm(target_cols, desc=f"{log_prefix.strip()}targets".strip(), leave=False)
        for col in target_iter:
            if col in copy_cols:
                continue
            ...
            if not is_anyana and not is_sensory:
                raise ValueError(f"Unexpected non-sensory target in Track 2 seed ensemble: {col}")
...
def run_one(
    ...
    track = 2
    LOGGER.info("Loading track %s data from %s", track, data_root)
    ...
    cfg = RunConfig(
        ...
        notes="Five-seed full-training TabPFN ensemble with probability averaging.",
    )
    ...
    if do_cv:
        ...
        for fold, (tr_idx, va_idx) in fold_iter:
            ...
            pred_va = _predict_seed_ensemble_probabilities(
                Xtr_f,
                ytr_f,
                Xva_f,
                target_cols,
        ...
        LOGGER.info("Writing CV metrics and OOF")
    ...
    pred_test = _predict_seed_ensemble_probabilities(
        Xtr,
        ytr,
        Xte,
        target_cols,
...
def main() -> None:
    ...
    print(f"[tabpfn_seed_ensemble] done -> {out_dir}")
