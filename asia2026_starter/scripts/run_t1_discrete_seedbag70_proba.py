...
def _parse_seeds(seeds_arg: Optional[str]) -> Optional[List[int]]:
...
def _make_seeds(n_seeds: int, seed: int, seeds_arg: Optional[str]) -> List[int]:
    parsed = _parse_seeds(seeds_arg)
    if parsed is not None:
        return parsed
    rng = np.random.default_rng(seed)
    out: List[int] = []
    while len(out) < n_seeds:
        cand = int(rng.integers(1, 2**31 - 1))
        if cand not in out:
            out.append(cand)
    return out
...
def _predict_seedbag_prob(
    ...
    n_total = len(Xtr_all)
    n_sub = int(round(train_frac * n_total))
    n_sub = max(2, min(n_sub, n_total))
    ...
    for seed in tqdm(seeds, desc=f"Seeds{log_prefix}"):
        ...
        LOGGER.info("%sSeed %d using %d/%d rows.", log_prefix, seed, n_sub, n_total)
        ...
        for col in tqdm(target_cols, desc=f"Targets seed={seed}{log_prefix}", leave=False):
            ...
            sub_idx = seed_to_idx[seed]
            Xtr_seed = Xtr_use[sub_idx]
            y_seed = y_int[sub_idx]
            if np.unique(y_seed).size < 2:
                Xtr_seed = Xtr_use
                y_seed = y_int
            model = _init_tabpfn_classifier("cuda")

            try:
                Xtr_safe, Xte_safe = _safe_reduce(Xtr_seed, Xte_use, seed=seed, do_pca=False)
                model.fit(Xtr_safe, y_seed)
                proba = model.predict_proba(Xte_safe)
            except Exception:
                Xtr_safe, Xte_safe = _safe_reduce(Xtr_seed, Xte_use, seed=seed, do_pca=True)
                model.fit(Xtr_safe, y_seed)
                proba = model.predict_proba(Xte_safe)
...
def run_one(
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Stage 1 seedbag70 proba. No GPU detected.")
    ...
    method = "t1_discrete_seedbag70_proba"
    ...
    if do_cv:
        ...
        LOGGER.info("Skipping CV/OOF metrics.")
...
def main() -> None:
    ...
    seeds = _make_seeds(args.n_seeds, args.seed, args.seed_list)
    LOGGER.info("Seed bag (%d): %s", len(seeds), seeds)
    ...
    print(f"[asia2026 t1 seedbag70 proba] done -> {out_dir}")
