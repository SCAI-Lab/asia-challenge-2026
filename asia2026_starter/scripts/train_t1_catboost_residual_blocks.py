...
def _prepare_base_features(df: pd.DataFrame, base_cols: List[str]) -> Tuple[pd.DataFrame, List[str]]:
...
def run_one(
    ...
    X_base, cat_cols = _prepare_base_features(Xtr, base_cols)
    ...
    if do_cv:
        ...
        for alpha in eval_lambdas:
            blend = oof_base + (alpha * oof_residual)
