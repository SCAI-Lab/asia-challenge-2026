...
from asia2026.tabpfn_model_t1 import tabpfn_predict_multioutput_t1
...
def run_one(
    ...
    cfg = RunConfig(
        ...
        notes="t1-safe tabpfn bag5: 5-fold bagging, single-thread, PCA retry",
    )
    ...
    def predict(_Xtr: pd.DataFrame, _ytr: pd.DataFrame, _X: pd.DataFrame, apply_copy_through: bool) -> np.ndarray:
        ...
        pred = tabpfn_predict_multioutput_t1(
            _Xtr,
            _ytr[target_cols],
            _X,
    ...
    print(f"[t1safe_bag5] oof filled rows: {int(oof_filled_rows.sum())}/{len(oof_filled_rows)}")
    print(f"[t1safe_bag5] imputed cells (all targets): {imputed_all}")
    print(f"[t1safe_bag5] imputed cells (sensory targets): {imputed_sensory}")
...
def main() -> None:
    ...
    print(f"[asia2026 t1 safe bag5] done -> {out_dir}")
