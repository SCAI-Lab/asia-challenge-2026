...
from asia2026.tabpfn_model_discrete import tabpfn_predict_multioutput_discrete
...
def run_one(
    ...
    cfg = RunConfig(
        ...
        notes="t2 tabpfn discrete: classifier-as-regressor for sensory targets",
    )
    ...
    def predict(_Xtr: pd.DataFrame, _ytr: pd.DataFrame, _X: pd.DataFrame) -> np.ndarray:
        ...
        pred = tabpfn_predict_multioutput_discrete(
            _Xtr,
            _ytr[target_cols],
            _X,
...
def main() -> None:
    ...
    print(f"[asia2026 t2 discrete] done -> {out_dir}")
