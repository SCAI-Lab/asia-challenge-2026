...
READY_MARKER="${TABIM_VENV}/.tabimpute_ready"
...
if [ ! -d "${TABIM_VENV}" ]; then
  ...
  echo "[15_prefetch_tabimpute] venv incomplete: ${TABIM_VENV}" >&2
  echo "[15_prefetch_tabimpute] Removing and recreating." >&2
  rm -rf "${TABIM_VENV}"
  python -m venv "${TABIM_VENV}"
fi
...
if marker_start not in txt or marker_end not in txt:
    raise SystemExit("[15_prefetch_tabimpute] Unexpected interface.py layout; aborting patch.")
...
print("[15_prefetch_tabimpute] Patched:", p)
PY
...
print("[15_prefetch_tabimpute] interface.py syntax OK")
PY
...
print("[15_prefetch_tabimpute] OK, out shape:", out.shape)
PY

date -u +"%Y-%m-%dT%H:%M:%SZ" > "${READY_MARKER}"

printf '%s\n' "[15_prefetch_tabimpute] TabImpute checkpoint prefetched. Cache under ${HF_HOME} / ${XDG_CACHE_HOME}."
